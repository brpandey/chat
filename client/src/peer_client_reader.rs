...
impl PeerReader {
    ...
    pub async fn handle_peer_read(&mut self, io_id: u16, eb: EventBus) -> Option<String> {
        ...
        loop {
            match &mut self.read {
                ...
                ReadHandle::B(ref mut client_rx) => br = Self::peer_read_b(&mut self.kill, &mut self.kill_rx,
                                                                           client_rx, io_id, &mut peer_name, &eb).await,
            }
    ...
    async fn peer_read_b(kill: &mut ShutdownTx, kill_rx: &mut ShutdownRx, client_rx: &mut Receiver<PeerMsg>,
                         io_id: u16, peer_name: &mut Option<String>, eb: &EventBus) -> bool {
        // Type B code
            ...
            Some(msg_b) = client_rx.recv() => {
                ...
                match msg_b {
                    ...
                    PeerMsg::Hello(name, msg) => {
                        ...
                        eb.notify(EventMsg::UpdatedSessionName(io_id, name.clone()))
                            .expect("Unable to send updated sesion msg");

                        *peer_name = Some(name);

                        let m = std::str::from_utf8(&msg).unwrap_or_default();
                        println!("< {}, to chat with: \\sw {}, for lobby: \\lob >",
                                 m, session_id(io_id));
                    },
                    PeerMsg::Note(msg) => {
